...
impl EventStore for TursoEventStore {
    ...
    async fn backfill_entity_vectors(
        ...
        let tx = conn
            ...
            .await
            .map_err(storage_error)?;
        // ARN-216 / ADR-0173: skip when the journal advanced past the sequence
        // these rows were derived from — a newer write's rows (write-behind or a
        // later reconcile) must not be overwritten by a stale load.
        let mut seq_rows = tx
            .query(
