...
async fn dst_boot_cycle_with_store_faults() {
    for seed in 0..NUM_SEEDS {
        ...
        let install_result = harness.install_app(TENANT, "project-management").await;
        ...
        // Fault injection covers install and dispatch. Disable it before
        // recovery and invariant reads so verification observes the complete
        // durable journal rather than injecting a new read fault into the
        // checker.
        let previous_event_faults = harness.sim_event_store.disable_faults();
...
async fn dst_boot_cycle_multi_tenant() {
    for seed in 0..NUM_SEEDS {
        ...
        let types_a = harness
            .install_app(tenant_a, "project-management")
            .await
        ...
        let types_b = harness
            .install_app(tenant_b, "temper-fs")
            .await
