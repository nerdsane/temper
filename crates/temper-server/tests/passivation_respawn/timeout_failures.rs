...
#[tokio::test]
async fn truncated_journal_tail_does_not_publish_or_repair_stale_timed_state() {
    let seed = 211;
    let (_guard, _clock, _id_gen) = install_deterministic_context(seed);
    let sim_store = SimEventStore::no_faults(seed);
    let tenant = TenantId::default();
    let entity_id = "legacy-initial-timed-truncated-tail";
    let actor_key = format!("{tenant}:InitialTimedTask:{entity_id}");
    ...
    sim_store
        ...
        .await
        .expect("seed a two-event tail that exits the timed state");
    let segments_before = sim_store.dump_segments(&actor_key);
    sim_store.restore_faults(SimFaultConfig {
    ...
    let state = common::build_single_tenant_state_with_store(
        sim_store.clone(),
        "legacy-timeout-truncated-journal-tail",
        "default",
        &[("InitialTimedTask", INITIAL_TIMED_TASK_IOA)],
    );
    assert!(
        state
            ...
            .await
            .is_err(),
        "hydration must reject a successful-looking prefix of the committed tail"
    );
    assert!(
        state.state_timeout_tracker.pending_snapshot().is_empty(),
        "a truncated tail must not arm a timeout from stale snapshot state"
    );
    ...
    assert_eq!(
        ...
        (1, legacy_snapshot_bytes),
        "an incomplete tail must not authorize a legacy snapshot rewrite"
    );
    assert_eq!(
        sim_store.dump_segments(&actor_key),
        segments_before,
    ...
    assert_eq!(
        sim_store.dump_journal(&actor_key).len(),
        3,
        "failed hydration must preserve the complete journal"
    );
}
