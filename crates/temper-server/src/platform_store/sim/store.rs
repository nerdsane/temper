//! Platform-store operations for the deterministic simulation backend.
...
impl PlatformStore for SimPlatformStore {
    async fn upsert_spec(
        ...
        let existing = inner.specs.get(&key);
        let changed = existing.is_none_or(|row| {
            row.content_hash != content_hash || row.csdl_xml.as_deref() != Some(csdl_xml)
        });
        let version = existing.map_or(1, |row| row.version + i64::from(changed));
        let committed = existing.is_some_and(|row| !changed && row.committed);
        inner.specs.insert(
            key,
            SpecRow {
                ...
                updated_at: format!("sim-version-{version}"),
            },
        );
        Ok(())
    }
    ...
    async fn load_tenant_constraints(&self) -> Result<Vec<TenantConstraintRow>, String> {
        ...
        Ok(inner
            .constraints
            .iter()
            .map(|(tenant, (source, version))| TenantConstraintRow {
                tenant: tenant.clone(),
                source: source.clone(),
                version: *version,
            })
    ...
    async fn replace_registry_restore_quarantines(
        ...
        quarantine::replace(self, None, source, active)
    }
    ...
    async fn replace_registry_restore_quarantines_for_tenant(
        ...
        quarantine::replace(self, Some(tenant), source, active)
    }
    ...
    async fn resolve_registry_restore_quarantines(
        ...
        quarantine::resolve(self, source, resolutions)
    }
    ...
    async fn acknowledge_registry_restore_quarantine(
        ...
        quarantine::acknowledge(self, tenant, entity_type, spec_version, constraint_version)
    }

    async fn load_registry_restore_quarantines(
        &self,
    ) -> Result<Vec<RegistryQuarantineRecord>, String> {
        quarantine::load(self, None, usize::MAX)
    }
    ...
    async fn load_registry_restore_quarantines_for_tenant(
        ...
        quarantine::load(self, Some(tenant), limit)
    }
    ...
    async fn upsert_tenant_constraints(
        ...
        inner.constraints.insert(
            tenant.to_string(),
            (cross_invariants_toml.to_string(), version),
        );
        Ok(())
    }
