//! Postgres implementation of the backend-neutral platform store.

use super::*;
use temper_store_postgres::{
    PostgresEventStore, PostgresInstalledAppRow, PostgresRegistrySourceSnapshot,
    PostgresSpecVerificationUpdate,
};

fn source_snapshot(source: &RegistrySourceSnapshot) -> PostgresRegistrySourceSnapshot {
...
fn quarantine_record(
    row: temper_store_postgres::PostgresRegistryQuarantineRow,
...
impl PlatformStore for PostgresEventStore {
    ...
    async fn load_tenant_constraints(&self) -> Result<Vec<TenantConstraintRow>, String> {
        let rows = sqlx::query_as::<_, (String, String, i64)>(
            "SELECT tenant, cross_invariants_toml, version FROM tenant_constraints ORDER BY tenant",
        )
        .fetch_all(self.pool())
        .await
    ...
    async fn replace_registry_restore_quarantines(
        ...
        let rows = active
            .iter()
            .map(
                |row| temper_store_postgres::PostgresRegistryQuarantineUpsert {
                    tenant: row.tenant,
                    entity_type: row.entity_type,
                    spec_version: row.spec_version,
                    constraint_version: row.constraint_version,
    ...
    async fn resolve_registry_restore_quarantines(
        ...
        let rows = resolutions
            .iter()
            .map(
                |row| temper_store_postgres::PostgresRegistryQuarantineResolution {
                    tenant: row.tenant,
                    entity_type: row.entity_type,
