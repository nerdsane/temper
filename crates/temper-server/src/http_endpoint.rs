...
///    literal prefix + suffix. `{owner}` matches any non-empty segment.
///    `{repo}.git` matches segments ending in `.git`, with the prefix
///    captured as `repo`. `v{ver}` matches `v1`, `v2.0`, etc.
