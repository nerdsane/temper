...
pub async fn handle_odata_post(
    ...
    match odata_path {
                ...
                .await
            {
                Ok(response) => {
                    ...
                    if let Some(obj) = body.as_object_mut() {
                        obj.insert(
                            "@odata.context".into(),
                            serde_json::json!(format!("$metadata#{name}/$entity")),
                        );
                        obj.insert(
                            "@odata.id".into(),
                            serde_json::json!(format!("{name}('{entity_id}')")),
                        );
                    }
                    ODataResponse {
        ...
        ODataPath::BoundAction { parent, action } => {
            ...
            // Cedar AuthZ check
            let authz_result = state.authorize(
                &[],
                &action,
                &entity_type,
            ...
            let response = match result {
                Ok(response) => {
                    if response.success {
                        ...
                        http_span
                            .set_status(Status::error(response.error.clone().unwrap_or_default()));
                        http_span.set_attribute(OtelKeyValue::new("http.status_code", 409i64));
                        odata_error(
                            StatusCode::CONFLICT,
                            "ActionFailed",
                ...
                Err(e) => {
                    http_span.set_status(Status::error(e.clone()));
                    http_span.set_attribute(OtelKeyValue::new("http.status_code", 500i64));
                    odata_error(StatusCode::INTERNAL_SERVER_ERROR, "DispatchError", &e)
                        .into_response()
                }
            };
