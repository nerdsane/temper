//! Mailbox handler for [`EntityActor`].
...
use crate::entity_actor::effects::{
    build_eval_context_with_xref, process_action_with_xref_and_field_mode,
    prune_transient_action_fields_from_state,
};
use crate::entity_actor::types::{
    EntityEvent, EntityMsg, EntityResponse, EntityState, MAX_EVENTS_SINCE_SNAPSHOT,
    MAX_ITEMS_PER_ENTITY,
};

use super::persist::ReplayPolicy;
use super::{EntityActor, recover_entity_state_from_store};

pub(crate) fn event_budget_workspace_id(state: &EntityState) -> String {
...
impl Actor for EntityActor {
    ...
    async fn pre_start(&self, _ctx: &mut ActorContext<Self>) -> Result<Self::State, ActorError> {
        ...
        if self.event_journal.is_some() && state.total_event_count == 0 {
            let initial_params =
                crate::entity_actor::effects::sanitize_action_params(&self.initial_fields)
                    .into_owned();
    ...
    async fn handle(
        ...
        match msg {
            ...
            EntityMsg::UpdateFields {
                ...
                let fields = crate::entity_actor::effects::sanitize_action_params(&fields);
                if replace {
