...
use crate::entity_actor::{EntityState, recover_entity_state_from_store};
...
impl ServerState {
    ...
    pub async fn populate_field_index_from_snapshots(&self, tenant: &TenantId) {
        ...
        // Phase 1: snapshot-based backfill.
        for (entity_type, entity_id) in &entities {
            let persistence_id = format!("{tenant}:{entity_type}:{entity_id}");
            match store.load_snapshot(&persistence_id).await {
                Ok(Some((_seq, snapshot_bytes))) => {
                    if let Ok(state) = serde_json::from_slice::<EntityState>(&snapshot_bytes) {
                        if let Err(e) = store
        ...
        // Phase 2: persistence replay for entities without snapshots.
