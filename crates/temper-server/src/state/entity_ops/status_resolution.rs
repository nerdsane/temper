//! Generation-aware entity-status resolution.
...
impl ServerState {
    ...
    pub async fn resolve_entity_status(
        ...
        self.resolve_entity_status_with_context(tenant, entity_type, entity_id, None)
            .await
    }

    /// Resolve entity status inside an already-admitted tenant generation.
    pub(crate) async fn resolve_entity_status_in_generation(
        &self,
        tenant: &TenantId,
        entity_type: &str,
        entity_id: &str,
        ...
        self.resolve_entity_status_with_context(tenant, entity_type, entity_id, Some(agent_ctx))
            .await
    }

    pub(super) async fn resolve_entity_status_with_context(
        &self,
        tenant: &TenantId,
        entity_type: &str,
        entity_id: &str,
        ...
        if let Ok(response) = self
            .get_tenant_entity_state_with_context(tenant, entity_type, entity_id, agent_ctx)
            .await
        {
