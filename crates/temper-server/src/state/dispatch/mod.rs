...
pub enum DispatchError {
    ...
    /// Operational collection-workflow mode rejected a new start.
    #[error("collection workflow unavailable: {0}")]
    CollectionWorkflowConflict(&'static str),
...
"#;
    ...
    fn collection_dispatch_state() -> crate::state::ServerState {
        ...
        // Keep the dispatcher installed so collection source actions take the
        // production commit path, but stop the background recovery worker so
        // this test can exercise control before the synthetic member completes.
        state
