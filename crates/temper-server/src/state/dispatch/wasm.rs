...
impl crate::state::ServerState {
    ...
    async fn dispatch_single_integration(
        ...
        let mut integration_config = match self.secrets_vault.as_ref() {
        ...
        if !integration_config.contains_key("temper_api_url")
            && let Some(api_url) = internal_api_base_url(self)
        {
            integration_config.insert("temper_api_url".to_string(), api_url);
        }
