...
pub struct AgentContext {
    ...
    /// Stable runtime generation inherited by immediate dispatch work.
    /// Delayed timers detach this lease and retain only its generation token.
    pub(crate) tenant_generation_lease: Option<crate::state::TenantGenerationLease>,
}
...
impl AgentContext {
    ...
    pub fn inherit_observability_from(mut self, parent: &AgentContext) -> Self {
        ...
        self.tenant_generation_lease = parent.tenant_generation_lease.clone();
        self
    }
    ...
    pub(crate) fn with_tenant_generation_lease(
        mut self,
        lease: crate::state::TenantGenerationLease,
    ) -> Self {
        self.tenant_generation_lease = Some(lease);
        self
    }

    pub(crate) fn detach_tenant_generation_lease(&mut self) -> Option<u64> {
        self.tenant_generation_lease
            .take()
            .map(|lease| lease.captured_generation())
    }

    /// Clone this context for immediate detached work while independently
    /// retaining the originating request generation (or its explicit
    /// publication-owned pending-generation token).
    pub(crate) fn fork_tenant_generation_lease(
        &self,
        tenant: &temper_runtime::tenant::TenantId,
    ) -> Option<Self> {
        let lease = self
            .tenant_generation_lease
            .as_ref()?
            .fork_immediate(tenant)?;
        let mut forked = self.clone();
        forked.tenant_generation_lease = Some(lease);
        Some(forked)
    }
}
