...
mod source_app;
use source_app::set_agent_source_app;
...
pub(super) async fn bootstrap_agents(
    ...
    for agent in agents {
        ...
        let existing_ids = match state.server.list_entity_ids_lazy(tenant_id, "Soul").await {
            Ok(ids) => ids,
            Err(error) => {
                tracing::error!(
                    tenant,
                    agent = %agent.name,
                    error = %error,
                    "failed to enumerate Soul entities during agent bootstrap"
                );
                continue;
            }
        };
