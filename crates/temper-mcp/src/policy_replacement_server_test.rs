...
#[tokio::test]
async fn large_native_amendment_commits_and_conflicts_on_real_temper() {
    let directory = std::env::temp_dir().join(format!("replacement-live-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir(&directory).unwrap();
    std::fs::set_permissions(&directory, std::fs::Permissions::from_mode(0o700)).unwrap();
    let store = TursoEventStore::new(
    ...
    let own_read = ctx
        ...
        .unwrap();
    assert_eq!(own_read.status(), reqwest::StatusCode::FORBIDDEN);
    let old_clause = "forbid(principal, action == Action::\"OldAction\", resource);";
    let new_clause = "forbid(principal, action == Action::\"ProtectedAction\", resource);";
    let old = format!(
        "{}{}",
        "// untouched bundle bytes\n".repeat(20000),
        old_clause
    );
    let new = old.replace(old_clause, new_clause);
    assert!(old.len() > 441883);
    store
        .save_policy("default", "legacy", &old, "fixture")
        .await
        .unwrap();
    temper_server::authz::load_and_activate_tenant_policies(&state, "default").await;
    let args = json!({"policy_id":"legacy","expected_hash":digest(&old),"edits":[{"old":old_clause,"new":new_clause}]});
    let before = store.load_policies_for_tenant("default").await.unwrap();
    let answer = async {
        let prompt = tokio::time::timeout(Duration::from_secs(10), rx.recv())
            .await
        ...
        assert!(prompt["params"].to_string().len() < 65536);
        for disclosed in [old_clause, new_clause, &digest(&old), &digest(&new)] {
            assert!(message.contains(disclosed));
        }
        assert!(!message.contains("untouched bundle bytes"));
        assert_eq!(
            api.entry("fixture-human", "legacy").await.unwrap()["cedar_text"],
            old
        );
    ...
    let call = json!({"jsonrpc":"2.0","id":91,"method":"tools/call","params":{"name":"request_policy_amendment","arguments":args}});
    let (result, ()) = tokio::join!(crate::protocol::dispatch_json_value(&mut ctx, call), answer);
    let result = result.unwrap();
    assert_eq!(result["result"]["isError"], false, "{result}");
    assert!(
