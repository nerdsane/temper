...
pub struct SpecDrivenActor {
    ...
    pub(crate) init_state: SpecActorState,
    /// Routing map: emit/trigger name → (target actor type, target action).
    pub(crate) routing: HashMap<String, (String, String)>,
    /// Leaked static refs for subscriptions() return.
    pub(crate) subscriptions_static: Vec<&'static str>,
}
...
impl SpecDrivenActor {
    ...
    pub fn from_automaton(
        ...
        effects::reject_unsupported_effects(&table)?;
        ...
        match result {
            Some(r) if r.success => {
                ...
                for effect in &r.effects {
                    self.apply_effect_inner(&mut actor_state, effect, ctx).await;
                }
