...
impl EventStore for PostgresEventStore {
    ...
    async fn append_with_index_rows(
        ...
        for key in key_rows {
            // Release markers (empty hash, ARN-238 / ADR-0172) claim nothing.
        ...
        for key in key_rows {
            ...
            .await
            .map_err(|e| PersistenceError::Storage(e.to_string()))?;
            // A release marker (ARN-238 / ADR-0172) only drops the prior row.
            if key.key_hash.is_empty() {
                continue;
            }
            crate::dbm::postgres_query!(
    ...
    async fn backfill_entity_keys(
        ...
        let mut tx = self
            ...
            .map_err(|e| PersistenceError::Storage(e.to_string()))?;
        for key in key_rows {
            // A release marker (empty hash, ARN-238 / ADR-0172) only drops the entity's row.
            if key.key_hash.is_empty() {
                crate::dbm::postgres_query!(
                    "DELETE FROM entity_key_index \
                     WHERE tenant = $1 AND entity_type = $2 AND key_name = $3 AND entity_id = $4",
