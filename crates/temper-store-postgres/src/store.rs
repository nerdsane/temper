...
impl EventStore for PostgresEventStore {
    ...
    async fn backfill_entity_vectors(
        ...
        let mut tx = self
            ...
            .await
            .map_err(|e| PersistenceError::Storage(e.to_string()))?;
        // ARN-216 / ADR-0173: the reconcile must not overwrite rows a NEWER live
        // write co-committed. Under READ COMMITTED a SELECT-then-DELETE guard is
        // not atomic (each statement gets a fresh snapshot), so the ordering is:
        // DELETE first — taking row locks that serialize any concurrent reconcile
        // of this entity behind this transaction — THEN check the journal sequence
        // under those locks and ROLL BACK if it advanced past the sequence these
        // rows were derived from. A live append committing before the DELETE is
        // caught by the post-DELETE check; one starting after it blocks on the
        // row locks until this transaction resolves.
        crate::dbm::postgres_query!(
            "DELETE FROM entity_vector_index \
             WHERE tenant = $1 AND entity_type = $2 AND entity_id = $3",
        )
