...
#[test]
fn repository_ioa_corpus_parses_and_round_trips_through_canonical_schema() {
    ...
    for path in paths {
        ...
        let canonical = match toml::to_string(&parsed) {
            ...
            Err(error) => {
                let bad_actions = parsed
                    .actions
                    .iter()
                    .filter_map(|action| {
                        toml::Value::try_from(action)
                            .err()
                            .map(|nested| format!("{} ({nested})", action.name))
                    })
                    .collect::<Vec<_>>();
                failures.push(format!(
                    "{}: canonical serialization: {error}; actions: {}",
                    relative.display(),
                    bad_actions.join(", ")
                ));
                continue;
        ...
        let reparsed =
            match parse_automaton_with_liveness(&canonical, LivenessEnforcement::WarnOnly) {
                ...
                Err(error) => {
                    failures.push(format!("{}: canonical parse: {error}", relative.display()));
                    continue;
                }
            };
        if toml::to_string(&reparsed).expect("reparsed automaton must serialize") != canonical {
            failures.push(format!(
