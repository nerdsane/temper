...
/// Remove declarations that the retired hand parser silently overwrote.
///
/// The differential compares integration behavior in historical files. These
/// snapshots contain unrelated duplicate declarations that the canonical
/// parser must reject, so the fixture repair removes only the overwritten copy.
