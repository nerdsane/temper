...
pub struct Action {
    ...
    /// the source). Webhook syntax is recognized but rejected by ADR-0171.
...
/// remain deserializable for an actionable ADR-0171 validation error, but
...
pub struct Integration {
    ...
    /// value is rejected until durable delivery exists (ADR-0171).
...
pub enum TriggerKind {
    ...
    /// but rejected until durable delivery exists (ADR-0171).
    Webhook,
}
...
/// - `Webhook`: rejected until the durable delivery contract in ADR-0171 exists.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct ActionTrigger {
    ...
    /// Historical outbound HTTP URL syntax retained so ADR-0171 can return a
    /// precise durability error for webhook declarations.
    #[serde(default)]
    pub url: Option<String>,
    /// Historical HTTP method syntax retained for the ADR-0171 error path.
    #[serde(default)]
    pub method: Option<String>,
    /// Historical HTTP headers retained for the ADR-0171 error path.
    #[serde(default)]
    pub headers: BTreeMap<String, String>,
    /// Historical HTTP request-body template retained for the ADR-0171 error
    /// path.
    #[serde(default)]
