...
impl FieldPredicate {
    ...
    fn from_toml_value(value: &toml::Value) -> Result<Self, PredicateParseError> {
        ...
        // Combinator branch.
        if has_any_of {
            let children = match table.get("any_of") {
                Some(v) => v,
                None => return Err(err("`any_of` key missing from table")),
            };
            let list = children
                .as_array()
        ...
        if has_all_of {
            let children = match table.get("all_of") {
                Some(v) => v,
                None => return Err(err("`all_of` key missing from table")),
            };
            let list = children
                .as_array()
        ...
        if has_not {
            let child_val = match table.get("not") {
                Some(v) => v,
                None => return Err(err("`not` key missing from table")),
            };
            let inner =
        ...
        if has_absent {
            let v = match table.get("absent") {
                Some(v) => v,
                None => return Err(err("`absent` key missing from table")),
            };
            let b = v
                .as_bool()
        ...
        if has_empty {
            let v = match table.get("empty") {
                Some(v) => v,
                None => return Err(err("`empty` key missing from table")),
            };
            let b = v
                .as_bool()
        ...
        // has_equals
        let v = match table.get("equals") {
            Some(v) => v,
            None => return Err(err("`equals` key missing from table")),
        };
...
/// Extract `[[field_invariant]]` sections from an IOA TOML source string.
///
/// The hand-written parser does not handle nested inline-table predicates,
/// so we delegate to `toml::from_str`. To keep this resilient against
/// unrelated TOML quirks elsewhere in the source (e.g. duplicate keys in
/// integration config), we first slice out only the `[[field_invariant]]`
/// sections and parse just those.
///
/// Parse errors are propagated — a silently dropped field invariant would
/// mean the constraint is not enforced at runtime, which is worse than a
/// loud parse failure.
pub fn extract_field_invariants(source: &str) -> Result<Vec<FieldInvariant>, String> {
    ...
    struct FieldInvariantWrapper {
        ...
        field_invariants: Vec<FieldInvariant>,
    }
    toml::from_str::<FieldInvariantWrapper>(&slice)
        .map(|w| w.field_invariants)
        .map_err(|e| format!("field_invariant: {e}"))
}
