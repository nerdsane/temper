...
RETIRED_PATTERN='pub enum ServerEventStore|from_server_event_store|impl PlatformStore for ServerEventStore'

VIOLATIONS="$(cd "$ROOT" && rg -n "$PATTERN" crates/temper-server/src crates/temper-cli/src \
...
RETIRED="$(cd "$ROOT" && rg -n "$RETIRED_PATTERN" crates/temper-server/src || true)"

if [[ -n "$RETIRED" ]]; then
  printf 'FAIL: retired storage compatibility surface is still present.\n' >&2
  printf '%s\n' "$RETIRED" >&2
  exit 1
fi
