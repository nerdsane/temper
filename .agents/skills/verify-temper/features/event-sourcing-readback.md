...
## Driving it
Every dispatch appends a `PersistenceEnvelope` (monotonic `sequence_nr`, event_type, payload, causation/correlation ids) via `store.append_with_index_rows(...)`. Index maintenance is backend-specific (confirmed in `temper-store-turso/src/store/event_store.rs`): on **postgres** the declared key-index (ADR-0153) and vector-index (ADR-0155) rows co-commit atomically with the event; on **turso** (the default) the key-index is NEVER maintained on write and is NOT backfilled either (deliberate, ADR-0153: turso keeps the no-op trait defaults - backfilling a not-live index would let a keyed miss wrongly read a present entity as absent, so turso must not be authoritative for keyed absence; postgres co-commits and is the authoritative keyed backend, so a read relying on the keyed oracle must use postgres, not turso), and the vector-index is **write-behind** - the event is appended first, then the vector rows follow in a separate retried write (safe for vectors; the partition lags until the next backfill reconcile if that write fails). Snapshots save every `TEMPER_SNAPSHOT_INTERVAL` events (default 100). On `pre_start`, `replay_events` loads the latest snapshot then folds events since, re-deriving effects through the transition table (`replay_effects` ignores guards - a committed event's guard already passed - and honors the stored `to_status`).
