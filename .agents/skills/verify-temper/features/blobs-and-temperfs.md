...
## Gotchas
- **The blob/TemperFS 503 is NOT storage-backend-gated.** The stale advice "503 when not on turso" is wrong: both turso and postgres stacks provide the query plane + metadata + blob API. The real blob-read 503s are transient/config: object store unconfigured (`BlobStoreUnavailable`, `content_addressed.rs`), object-store read error (`BlobMediaUnavailable`, `blob_media.rs`), and file-stream projection read failure (`FileReadIndexUnavailable`, `stream_fast_path.rs`). Report the 503 by its code, not by the backend. (The ADR-0048 `503 + Retry-After` is a separate thing - `DispatchError::Transient` on the dispatch path, `bindings.rs:359` - not a blob read.)
