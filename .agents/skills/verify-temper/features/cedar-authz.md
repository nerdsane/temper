...
```
Or drive the whole approve loop with the CLI: `TEMPER_API_KEY=… cargo run -p temper-cli -- decide --port 3600 --tenant default`.

Policy plane lives under `/api/tenants/{tenant}/policies` (GET/PUT, `/rules`, `/list`, `/create`, `/entry/{id}` PATCH/DELETE, `/suggestions`) and decisions under `/api/tenants/{tenant}/decisions` (list, `/stream` SSE, `/{id}`, `/{id}/approve`, `/{id}/deny`). `POST /api/authorize` is a non-blocking pre-flight probe: it always returns 200 with `{allowed:true|false, decision_id?, reason?}`.
