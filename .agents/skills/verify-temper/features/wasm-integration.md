...
## What proves it
Three observable signals: a row in the `wasm_invocation_logs` table (module, trigger_action, callback_action, success, duration_ms; `temper-store-turso/src/schema.rs`); an `integration_complete` observe event on the entity (`{module, result, callback_action}`); and, if a callback action fired, the source entity's state advanced (read it back over OData).
