...
export default function NarrativeSection({
  ...
  const textContent = (
    <div className="flex-1 min-w-0 py-[25vh] max-[900px]:py-20 max-[900px]:pb-10 max-[900px]:w-full max-[900px]:order-1">
      <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-teal-400 mb-4 block">
      ...
      {paragraphs.map((p, i) => (
        <p key={i} className="text-[15px] text-zinc-400 mb-6 max-w-[480px] leading-[1.7]">
          {p}
        </p>
      ))}
    </div>
  ...
  const vizContent = (
    <div className="flex-1 min-w-0 sticky top-0 h-screen flex items-center justify-center z-10 max-[900px]:relative max-[900px]:h-auto max-[900px]:min-h-[320px] max-[900px]:py-5 max-[900px]:w-full max-[900px]:order-2 max-[900px]:mb-[60px]">
      <div className="w-full max-w-[380px] aspect-square bg-white/[0.02] backdrop-blur-xl border border-white/[0.04] rounded-[3px] relative overflow-hidden flex items-center justify-center transition-[border-color] duration-250 hover:border-white/10 max-sm:max-w-full">
        {children}
      </div>
    </div>
  );
