...
### Portal Registration — Keep Apps Discoverable

There is a shared workspace portal at `~/workspace/apps/portal/`. It shows all agent apps in one place with live status and widget data. **Every app must register here — otherwise your human can't find it.**

**The portal reads URLs from Temper, not from hardcoded config.** This means you must fire `GoLive` with a `Url` param whenever you start your server + tunnel.

#### One-time: Add your app to the portal config

In `~/workspace/apps/portal/index.html`, add an entry to the `APPS` array:

```js
{
  id:     'your-app-id',         // matches the AgentApp entity name in Temper
  name:   'Short Name',          // displayed on the card
  agent:  'your-agent-name',     // e.g. 'kiki', 'jiji', 'calcifer'
  emoji:  '🌿',
  port:   8081,                  // fallback for local-only access
  color:  '#4ade80',             // your agent's signature color
  tenant: 'your-app-id',        // Temper tenant for widget data
  fetch:  fetchYourWidgetData,   // async fn returning [{label, value}] or null
},
```

Also add a `fetchYourWidgetData` function that queries your Temper tenant for live stats to show in the widget. Return `null` if you have no data yet.

#### Every session: start server → tunnel → register

```bash
# 1. Start server
nohup python3 ~/workspace/apps/your-app/serve.py > /tmp/your-app.log 2>&1 &
sleep 1

# 2. Start tunnel
nohup cloudflared tunnel --url http://localhost:8081 --no-autoupdate > /tmp/cf-your-app.log 2>&1 &
sleep 8
URL=$(grep -o 'https://[^ ]*trycloudflare.com' /tmp/cf-your-app.log | tail -1)

# 3. Fire GoLive with the URL — portal picks this up automatically
ENTITY_ID="your-entity-id"   # the AgentApp entity id in the portal tenant
curl -X POST "http://localhost:3001/tdata/AgentApps('$ENTITY_ID')/Temper.GoLive" \
  -H "Content-Type: application/json" \
  -H "X-Tenant-Id: portal" \
  -d "{\"Url\": \"$URL\"}"

# 4. DM your human the URL
```

The portal fetches the URL from the last `GoLive` event in the entity trajectory. No config file editing needed — the state machine IS the config.

#### Finding your entity ID

```bash
curl -s http://localhost:3001/tdata/AgentApps -H "X-Tenant-Id: portal" | \
  python3 -c "import sys,json; [print(e['entity_id'], e['fields'].get('name')) for e in json.load(sys.stdin).get('value',[])]"
```

If your app doesn't have an entity yet:
```bash
curl -X POST http://localhost:3001/tdata/AgentApps \
  -H "Content-Type: application/json" -H "X-Tenant-Id: portal" \
  -d '{"name": "your-app-id"}'
```

#### When your session ends

Fire `GoOffline` so the portal shows the right status:
```bash
curl -X POST "http://localhost:3001/tdata/AgentApps('$ENTITY_ID')/Temper.GoOffline" \
  -H "Content-Type: application/json" -H "X-Tenant-Id: portal" -d '{}'
```

**If you skip these steps, the portal card shows a stale URL or no URL. Your human gets a broken link. That's on you.**
