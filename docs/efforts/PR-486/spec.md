...
## Foundry host boundary

Foundry keeps deployment credentials on its API server and explicitly unsets connector approver credentials. In this mode the trusted launcher sets `TEMPER_MCP_POLICY_APPROVAL_RELAY=1`. The native tool submits only a proposal to the configured gateway; the host records a run-bound Ask containing the exact policy before/after and version hashes. Only the signed-in human answer route can apply it using the server-held approver. The agent cannot mint this Ask through generic elicitation, supply consent in the proposal, call the administrative endpoint, or change the connection/tenant. Repeating a proposal reads the same request and never writes policy.

The host integration and pinned connector are delivered together in `arni-labs/foundry`. A standalone direct-server test is not evidence that the gateway flow works; the local gateway proof must include a signed-in fixture human and demonstrate rejection of agent tickets at the answer boundary.

## Concurrency boundary

The policy API and decision approvals share the existing process-local mutation lock through durable commit and activation. The database hash comparison remains authoritative for other instances and out-of-band writers. This change does not add cross-instance cache synchronization; the receipt verifies the serving instance's active policy and durable row at that operation.
