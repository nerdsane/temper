...
New Redis appends update the historical entity set, tenant and typed live sorted sets, tombstone set, discovered-membership set, and index-version metadata in the same append Lua script. Listing therefore never scans journals for upgraded tenants. Legacy tenants migrate through a Redis-side pending set plus per-entity journal cursors. A scan call first rejects a compact encoding whose cardinality exceeds the fixed page budget, because Redis returns the complete compact Set regardless of `COUNT`. Hashtable scans request `COUNT 1`; because `COUNT` remains only a hint, every result is checked against an independent 128-member ceiling before per-member work. An exceptional over-ceiling result fails explicitly without advancing the cursor and requires an offline migration instead of silently violating the request-path budget. Every accepted page is durably copied into a bounded spill list before cursor advancement. Later calls drain at most the reference budget from that spill and decode at most one fixed event chunk, preserving the legacy `SMEMBERS` view while guaranteeing monotonic progress through every accepted cursor page. Scalar and null payloads are valid non-target payloads. Until every historical reference is discovered, every pending journal is classified, and an atomic cardinality check confirms every discovered reference is represented as live or terminal, the bounded listing returns an explicit retryable migration-incomplete error rather than an authoritative-looking subset.

Completion is revalidated on reads. If a mixed-version writer adds a historical reference without the new indexes, the historical/discovered mismatch atomically clears completion and resumes bounded sampling. Upgraded appenders converge with migration because both update the same discovered and live/tombstone metadata atomically. Segment records remain derived metadata: once the append Lua script commits the journal, a later segment-reconciliation failure is logged for repair but cannot turn the committed append into an acknowledged failure.
...
- **Legacy Redis writers.** Completion validation detects historical references added without upgraded index metadata and resumes migration. Rolling deployments must still converge writers promptly because an old writer mutating an already-known journal has no legacy global mutation counter; listing revalidates each candidate from its durable classified journal cursor before trusting live membership.
