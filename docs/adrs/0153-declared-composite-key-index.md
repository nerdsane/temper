...
## Reconcile triggers the re-key (2026-07-01, ARN-68 deploy-path fix)

Deploying the key-set-aware watermark + the `ws_path` key surfaced a boot-ordering race in prod that the local proof had missed (it registered the final spec straight from the IOA, never the add-a-key-to-an-already-installed-type transition). The once-per-boot key-index backfill (`spawn_key_index_backfill`, delayed 30s) fires BEFORE the os-app reconcile registers a newly-declared `[[key]]` — the reconcile runs late (post-boot, ~11 min in prod). So the backfill sees only the old declared key-set, (correctly, being key-set aware) skips, and the added key never backfills for existing entities; the read then scans → 413. A first-key on a fresh type (File `ws_path`) worked because it was registered at initial install; an added key on an installed type (Directory `ws_path` after `name_parent`) did not.

Fix: `install_os_app_with_plan` (temper-platform) now spawns the key-set-aware re-key (`populate_key_index_from_snapshots`) AFTER it registers changed specs (`plan.specs && (added|updated non-empty)`). Because the re-key is key-set aware it re-scans only types whose declared key-set actually changed and skips the rest via the watermark; it runs whenever/however late the reconcile runs, closing the race deterministically instead of relying on a delay outrunning the reconcile. Verified through the exact prod path on real Postgres at the normal 30s delay: install Directory with `name_parent`, add `ws_path`, reboot → existing dirs re-key (watermark `name_parent,ws_path`), `Path eq '/souls'` → 200, a real soul write lands. Follow-up: a temper-platform integration test driving install-then-add-key (the spawn's async effect makes it poll-based; the underlying re-key is unit-covered in `keyed_existence.rs`).
