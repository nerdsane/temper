...
Tests (against the sim store, which co-commits + watermarks like prod Postgres): `declared_keys_resolve_from_registry_not_just_transition_tables` (A), `key_index_backfill_keys_store_entities_absent_from_the_lazy_index` (B), `keyed_miss_returns_empty_without_scan_413_once_watermarked` (C — proves the read returns empty instead of 413 once watermarked). The standalone `temper serve` boot path now also runs the key-index backfill per tenant (it previously ran only the field-index backfill); TemperPaw already wired it via `spawn_key_index_backfill`.
