...
- Partially superseded by: ADR-0176 (outbound webhook acceptance and expansion only)
- Related:
...
- `kind = "webhook"` — outbound HTTP syntax. **Rejected by ADR-0176 until durable delivery exists.**
...
**Webhook boundary (superseded by ADR-0176).** The historical runtime follow-up
...
`kind = "wasm"` triggers are modeled symbolically during verification — the WASM execution is opaque — but their `on_success` / `on_failure` dispatches participate in joint verification like any other entity-kind trigger. Webhook triggers are rejected before verification (ADR-0176).
