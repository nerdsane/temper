...
| 14 | IOA webhook integration has no durable runtime | temper-spec, temper-platform | **BOUNDED** | ADR-0176 rejects outbound IOA webhook declarations before verification. The standalone platform engine retains directly configured retry/dead-letter behavior, but is not an IOA ingestion path or durable outbox. |
